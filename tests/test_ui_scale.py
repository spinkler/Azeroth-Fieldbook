...
scale:Set(0.5); assert(book.value==0.45 and db.uiScale==0.5)
...
scale:Set(2); assert(db.uiScale==1.5)
scale:Set(0.1); assert(db.uiScale==0.5)
scale:Initialize(db); assert(book.value==0.45)
scale:Set(1); assert(book.value==0.9 and lateWindow.value==1)
db.uiScale=nil; scale:Initialize(db); assert(scale:Get()==1)
''')
